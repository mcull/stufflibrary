...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getProviders(), getSession()]).then(
      ([availableProviders, session]) => {
        setProviders(availableProviders);

        // In dev/localhost, allow any authenticated user through to /admin
        ...
        if (githubUsername === 'mcull') {
          ...
          if (!availableProviders?.github) {
            setError(
              'GitHub authentication not configured. Please set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET environment variables.'
            );
          }
          setLoading(false);
        }
          ...
          {isDev ? (
            <div className="text-center space-y-3">
              <p className="text-sm text-gray-600">
                Local dev detected. You can sign in with email code and access
                admin in dev.
              </p>
              <a
                href="/auth/signin"
