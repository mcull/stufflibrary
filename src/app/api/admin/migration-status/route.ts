...
export async function GET(request: NextRequest) {
  try {
    ...
    const isAdmin = authHeader === `Bearer ${process.env.ADMIN_API_KEY}` || 
                   request.nextUrl.searchParams.get('admin_key') === process.env.ADMIN_API_KEY;
    ...
    const tables = await db.$queryRawUnsafe<{ table_name: string }[]>(tablesQuery);
    ...
    const coordinateColumns = await db.$queryRawUnsafe<{ column_name: string }[]>(coordinatesQuery);
    ...
    const status = {
      ...
      database: {
        ...
        tables: {
          found: tableNames,
          missing: ['libraries', 'library_members', 'item_libraries', 'users', 'addresses']
        ...
        counts: {
          ...
          libraries: counts[1].status === 'fulfilled' ? counts[1].value : 'error',
          invitations: counts[2].status === 'fulfilled' ? counts[2].value : 'error',
        }
      },
      health: {
        overall: tableNames.includes('libraries') && tableNames.includes('users') ? 'healthy' : 'unhealthy',
        canCreateLibraries: tableNames.includes('libraries') && tableNames.includes('library_members'),
        hasGoogleMaps: coordinateColumns.length === 2,
        criticalTablesPresent: ['users', 'libraries', 'library_members'].every(t => tableNames.includes(t))
      }
    };
